import agencyBlueprint from "../blueprints/agency.formBlueprint.json";
import assetBlueprint from "../blueprints/asset.formBlueprint.json";
import collaborationBlueprint from "../blueprints/collaboration.formBlueprint.json";
import companyBlueprint from "../blueprints/company.formBlueprint.json";
import eventBlueprint from "../blueprints/event.formBlueprint.json";
import institutionBlueprint from "../blueprints/institution.formBlueprint.json";
import locationBlueprint from "../blueprints/location.formBlueprint.json";
import organizationtBlueprint from "../blueprints/organization.formBlueprint.json";
import personBlueprint from "../blueprints/person.formBlueprint.json";
import scriptBlueprint from "../blueprints/script.formBlueprint.json";
import storyBlueprint from "../blueprints/story.formBlueprint.json";
import userBlueprint from "../blueprints/user.formBlueprint.json";
import videoBlueprint from "../blueprints/video.formBlueprint.json";

export const blueprintMap = {
  agencies: agencyBlueprint,
  assets: assetBlueprint,
  collaborations: collaborationBlueprint,
  companies: companyBlueprint,
  events: eventBlueprint,
  institutions: institutionBlueprint,
  locations: locationBlueprint,
  organizations: organizationtBlueprint,
  persons: personBlueprint,
  scripts: scriptBlueprint,
  stories: storyBlueprint,
  users: userBlueprint,
  videos: videoBlueprint,
};
